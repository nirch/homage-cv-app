...
static int	bImage_fill( image_type *bim, int a0[], int a1[] );

static int	bImage_row( image_type *bim, int a0[], int a1[] );


static int	bImage_dilate_L0( image_type *bim, image_type *eim, int a[], int n, segC_type ac[] );

 static int	bImage_dilate_L( image_type *bim, image_type *eim, int a[], int n, segC_type ac[100] );

 static int	bImage_dilate_R( image_type *bim, image_type *eim, int a[], int n, segC_type ac[100] );


 static  int	bImage_state( segC_type ac0[], segC_type *ac1, int nC, int Dx, int Av, int nc[2] );

 static int	bImage_erode( image_type *bim, image_type *eim, int a0[], int a1[], segC_type ac0[], segC_type ac1[], int Dx, int Av );

 static int	bImage_erode_up( image_type *bim, image_type *eim, int a0[], int a1[], segC_type ac0[], segC_type ac1[], int Dx, int Av );
...
int	CUnBackgroundM::ProcessInitBackground( image_type *sim, image_type *mim )
{
...
bImage_dilate_L0( image_type *bim, image_type *eim, int a[], int n, segC_type ac[] )
{
	...
	for( i = 1; i < bim->height-1 ; i++  ){
		...
		for( k = 1 ; k < 5 ; k++ ){
			...
			if( bImage_segC( bim, eim, i, a[i]-k+dj, n, &c ) < 0 )
				continue;
...
int	k,	j0;
	...
	for( k = 0 ; ; k++ ){
		...
		if( bImage_segC( bim, eim, i, c0->j0-k, c0->n, c ) < 0 )
			continue;
...
bImage_dilate_R( image_type *bim, image_type *eim, segC_type *s0, int i, segC_type *c0, int dk, segC_type *c )
{
	...
	for( k = 0 ; ; k++ ){
		...
		if( bImage_segC( bim, eim, i, j0, c0->n, c ) < 0 )
			continue;
...
bImage_erode( image_type *bim, image_type *eim, int a0[], int a1[], segC_type ac0[], segC_type ac1[], int Dx, int Av )
{
	...
	for( i = 1 ; i < bim->height-1 ; i++, bp += bim->width ){
		...
		for( j = j0+1, cp = bp0+1 ; j < j11 ; j++, cp++ ){
			if( bImage_segC( bim, eim, i, j-n, n, &c ) < 0 )
				break;
		...
		if( j1 < bim->width-1 - 0 ){
			...
			for( j = j1-1, cp = bp1-1 ; j > j0 ; j--, cp-- ){
				if( bImage_segC( bim, eim, i, j, n, &c ) < 0 )
					break;

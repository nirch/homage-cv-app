...
vl2fA_type *
vl2fA_copy( vl2fA_type *bavl, vl2fA_type *avl )
{
...
vl2fA_type *
vl2fA_from( vl2f_type *av[], int nV, vl2fA_type *avl )
{
	int	i;

	avl = vl2fA_realloc( avl, nV );
...
vl2fA_type *
	vl2fA_transform_lt2( vl2fA_type *bavl, lt2_type *lt, vl2fA_type *avl )
{
	int i;
	...
	for( i = 0 ; i < bavl->nA ; i++ ){
		...
		vl2f_transform_lt2( &bavl->a[i], lt, &avl->a[i]);
	}
...
void
vl2f_transform_lt2( vl2f_type *bvl, lt2_type *lt, vl2f_type *vl )
{

	LT2_F( *lt, bvl->p, vl->p );

	vl->v.x = lt->a0 * bvl->v.x + lt->b0*bvl->v.y;
	vl->v.y = lt->a1 * bvl->v.x + lt->b1*bvl->v.y;

	float t = hypot( vl->v.x, vl->v.y );

...
void	imageLabelUS_createN( image_type *im, bwLabel_type **tbw, int *tnBw );
...
void
	imageLabelUS_createN( image_type *im, bwLabel_type **tbw, int *tnBw )
{
	bwLabel_type	*bw;
	int	nBw;
	...
	nBw = 0;
	lbm_type *b0 = (lbm_type *)malloc( im->width * sizeof(lbm_type) );
	lbm_type *b1 = (lbm_type *)malloc( im->width * sizeof(lbm_type) );
	...
	for( i = 0 ; i < im->height ; i++ ){
		...
		for( j = 0 ; j < im->width ;  ){
			...
			if( id < 0  ){
				...
				for( k = k0 ; k <= k1 ; k++ ){
					if( b0[k].id > id ){
						bwLabel_merge( bw, nBw,  b0[k].id , id );
					}
				}
				k = k1;
			}

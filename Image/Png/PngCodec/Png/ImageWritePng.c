...
/****** write_png *****
Description:Writes an image_type image into a png file
Params:	file_name 
		image		
Returns:	number of bytes written
	  -1 - error opening the file
	  -2 - error reading the file
	  -3 - error creating structures	  
*****************************/
int image_write_png( image_type *image,char *file_name)
{
   ...
   u_char *tp;
   char dir[2048],name[2048],ext[267],newName[2048];
   gp_filename_split(file_name,dir,name,ext);
   sprintf(newName,"%s/%s.png",dir,name);
					  ...
					  char *file_name)
{
   ...
   /* ... set palette colors ... */
   if( palette != NULL ){
	   struct png_color_struct apl[256];
	   int	i;
	   for( i = 0 ; i < palette->nColor ; i++ ){
		   apl[i].red = palette->data[i].Red;
		   apl[i].green = palette->data[i].Green;
		   apl[i].blue = palette->data[i].Blue;
	   }


		png_set_PLTE(png_ptr, info_ptr, (struct png_color_struct *)apl, palette->nColor);
   }
...
u_int	*sp;
	...
	*/
	...
	png_set_IHDR(png_ptr, info_ptr, image->column, image->row, 8, PNG_COLOR_TYPE_RGB_ALPHA,
		PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
	...
	*/
	...
	for (k = 0; k < image->row ; k++)
	{
		...
		row_pointers[k] = (u_char*)malloc(4*col * sizeof (u_char ));
		tp = row_pointers[k];

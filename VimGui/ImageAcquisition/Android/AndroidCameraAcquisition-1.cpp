...
void CCameraAcquisition::SetFrameSize( int width, int height )
{
	m_rWidth = width;
	m_rHeight = height;
...
int CCameraAcquisition::Get( image_type **im, int iFrame )
{
	...
	// crop
	if( (*im)->width > m_rWidth ){
		int	x0 = 0.5*( (*im)->width - m_rWidth);
		int	y0 = 0.5*( (*im)->height - m_rHeight);

		m_image = image_crop( (*im), x0, y0, m_rWidth, m_rHeight, m_image );
		*im = m_image;
	}
...
int	i;
	...
	m_cameraGrabber->SetGain( 0, CG_AUTO );
	...
	m_cameraGrabber->GetGain( &value, &flag );
	m_cameraGrabber->SetGain( value, CG_MANUAL );
